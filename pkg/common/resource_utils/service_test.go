...
func TestBuildExternalService_ForStarRocksCluster(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotFeService := BuildExternalService(object, src.Spec.StarRocksFeSpec, map[string]interface{}{}, map[string]string{}, map[string]string{})
			equal(gotFeService, tt.wantFeService)
			gotBeService := BuildExternalService(object, src.Spec.StarRocksBeSpec, map[string]interface{}{}, map[string]string{}, map[string]string{})
			equal(gotBeService, tt.wantBeService)
			gotCnService := BuildExternalService(object, src.Spec.StarRocksCnSpec, map[string]interface{}{}, map[string]string{}, map[string]string{})
			equal(gotCnService, tt.wantCnService)
		})
	}
}
